...
class _Node():
    ...
    while True :
        if x >= self.key :
            ...
            if self.left != None :
                ...
                return self.key
        if self.right is None and self.key <= x :
            return snode.key
  ...
  def _range(self, output, low, high):
    ...
    if self.left is not None :
        output.extend(self.left._range(loutput, low, high))
    if self.right is not None :
        output.extend(self.right._range(routput, low, high))
...
that are <= x and > x, respectively."""
    if self.key <= x :
        root = _Node(self.key, self.left, self.right)
        lroot = None
        if root.right is not None :
            lroot, rroot = root.right._split(x)
        else :
            rroot = None
        root.right = lroot
        return root, rroot
    ...
    if self.key > x :
        ...
        if root.left is not None :
            lroot, rroot = root.left._split(x)
            root.left = rroot
    return lroot, root
# --------------------------------------------------------------------
